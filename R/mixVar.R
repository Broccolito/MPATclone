#' @export
mixVar <- function (Z.vec, Sigma, method = "davies") 
{
    ...
    I = diag(K)
    H = 1/K * J %*% t(J)
    varUsq = 2 * sum(diag(R.inv %*% J %*% t(J) %*% R.inv %*% 
        J %*% t(J)))
    Lambda.tau = (I - H) %*% R.inv %*% R.inv %*% (I - H)
    varTau = 2 * sum(diag(Lambda.tau %*% Sigma %*% Lambda.tau %*% 
        Sigma))
    phi = varTau/(varUsq + varTau)
    Lambda.phi.mat = phi * R.inv %*% J %*% t(J) %*% R.inv + (1 - 
        phi) * (I - H) %*% R.inv %*% R.inv %*% (I - H)
