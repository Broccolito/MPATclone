#' @export
mixFisher <- function (Z.vec, Sigma, method = "davies") 
{
    ...
    I = diag(K)
    H = 1/K * J %*% t(J)
    U.mu = (t(J) %*% R.inv %*% z.tmp)^2/(t(J) %*% R.inv %*% J)
    pval.mu = pchisq(U.mu, df = 1, lower.tail = FALSE)
    mu.hat = mean(z.tmp)
    U.tau = t(z.tmp - mu.hat) %*% R.inv %*% R.inv %*% (z.tmp - 
        mu.hat)
    lamb = eigen(Sigma)$values
    P = eigen(Sigma)$vectors
    Sigma.sqrt = P %*% sqrt(diag(lamb)) %*% t(P)
    mat.tmp = Sigma.sqrt %*% (I - H) %*% R.inv %*% R.inv %*% 
        (I - H) %*% Sigma.sqrt
    ...
    pval.mixFisher = pchisq(mixFisher, df = 4, lower.tail = FALSE)
    p_group = pval.mu
